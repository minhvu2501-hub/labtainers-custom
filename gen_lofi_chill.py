...
if __name__ == "__main__":
    ...
    save(os.path.join(out_dir, "lofi_clean.wav"), cover)

    # 2. LSB 1-bit: C2 beacon
    lsb1 = embed_lsb1(cover, PAYLOAD_LSB1)
    save(os.path.join(out_dir, "lofi_lsb1.wav"), lsb1)

    # 3. LSB 4-bit: PowerShell command
    lsb4 = embed_lsb4(cover, PAYLOAD_LSB4)
    save(os.path.join(out_dir, "lofi_lsb4.wav"), lsb4)

    # 4. Hash/pseudo-random: API token
    lsb_hash = embed_hash(cover, PAYLOAD_HASH)
    save(os.path.join(out_dir, "lofi_hash.wav"), lsb_hash)

    print()
    print("  PAYLOADS EMBEDDED:")
    print(f"  lofi_lsb1.wav  -> LSB 1-bit  : '{PAYLOAD_LSB1[:30]}...'")
    print(f"  lofi_lsb4.wav  -> LSB 4-bit  : '{PAYLOAD_LSB4[:30]}...'")
    print(f"  lofi_hash.wav  -> Hash LSB   : '{PAYLOAD_HASH[:30]}...'")
    print("=" * 55)
